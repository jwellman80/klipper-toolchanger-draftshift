...
TOOLCHANGER_PATH="${HOME}/printer_data/config/stealthchanger/toolchanger"
TOOL_PATH="${HOME}/printer_data/config/stealthchanger/tools"
set -eu
export LC_ALL=C
...
function link_macros {
    ...
    for file in "${INSTALL_PATH}"/macros/*.cfg; do
        if ! ln -sfn ${file} "${TOOLCHANGER_PATH}"/; then
            echo " failed!"
            exit -1
        fi
    done
...
function link_examples {
    echo -n "[INSTALL] Linking in examples to Klipper..."
...
function copy_examples {
    ...
    for file in "${INSTALL_PATH}"/examples/*.cfg; do
        if ! cp -n ${file} "${TOOLCHANGER_PATH}"/; then
            echo " failed!"
            exit -1
        fi
    done
